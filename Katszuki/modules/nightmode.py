...
from Katszuki.modules.sql.night_mode_sql import add_nightmode, rmnightmode, get_all_chat_id, is_nightmode_indb
from telethon.tl.types import ChatBannedRights
from apscheduler.schedulers.asyncio import AsyncIOScheduler 
from telethon import functions
from Katszuki.events import register
from Katszuki import OWNER_ID
from Katszuki import telethn as tbot
...
async def profanity(event):
    ...
    if not input:
        if is_nightmode_indb(str(event.chat_id)):
                await event.reply(
                    "✅NightMode is Enabled for this Chat"
                )
                return
        await event.reply(
    ...
    for pro in chats:
        try:
            await tbot.send_message(
              int(pro.chat_id), "✅ Night Mode Started !\n\n 2:00 Am, Group Is Closing Till 6 Am. \n**Powered By @Katszuki_Bot**"
            )
            await tbot(
