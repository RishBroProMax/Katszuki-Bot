...
import Katszuki.modules.sql.feds_sql as sql
from Katszuki import DRAGONS, EVENT_LOGS, LOGGER, OWNER_ID, TIGERS, WOLVES, dispatcher
...
from Katszuki.modules.helper_funcs.string_handling import markdown_parser
...
def new_fed(update: Update, context: CallbackContext):
    ...
    if not fednam == "":
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact @EmoBotSupport or @KatszukiSupport if the problem persist."
            )
            return
