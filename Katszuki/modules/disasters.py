...
from Katszuki import DEMONS, DEV_USERS, DRAGONS, OWNER_ID, TIGERS, WOLVES, dispatcher
from Katszuki.modules.helper_funcs.chat_status import dev_plus, sudo_plus, whitelist_plus
from Katszuki.modules.helper_funcs.extraction import extract_user
from Katszuki.modules.log_channel import gloggable

ELEVATED_USERS_FILE = os.path.join(os.getcwd(), "Maxrobot/elevated_users.json")
...
def addpiro(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DEMONS:
        rt += "Requested AU to promote a Demon Disaster to Pro Max Developer."
        data["supports"].remove(user_id)
        DEMONS.remove(user_id)

    if user_id in WOLVES:
        rt += "Requested AU to promote a Wolf Disaster to Pro Max Developer."
    ...
    update.effective_message.reply_text(
        rt
        + "\nSuccessfully set Disaster level of {} to Pro Max Developer!".format(
